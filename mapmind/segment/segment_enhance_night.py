...
def smooth_dark_enhancement(
    image,
    gamma_min=0.5,
    gamma_max=1.0,
    alpha=2.0,
    keep_threshold=100.0,
    blur_strength=15,
):
    """
...
def edge_preserving_denoise(img_array, blur_strength=10, edge_preserve_weight=0.75):
    ...
    img_denoised = cv2.bilateralFilter(img_array, d=blur_strength, sigmaColor=75, sigmaSpace=75)

    # Preserve edges by blending original and denoised images
    mask = (edges_dilated > 0).astype(np.float32)[..., np.newaxis]
    enhanced_img = (
        (1 - edge_preserve_weight) * img_denoised
        + edge_preserve_weight * img_array * mask
