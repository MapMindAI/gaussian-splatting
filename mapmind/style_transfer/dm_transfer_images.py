...
def stylize_blocks(style_model, content_image, divide=2, device = torch.device("cuda")):
    ...
    for i in range(len(offsets)):
        ...
        offset = offsets[i]
        sub_image = content_image[image_roi[0]+offset[0]:image_roi[1]+offset[1], image_roi[2]+offset[2]:image_roi[3]+offset[3], :]
        sub_style_image = stylize_blocks(style_model, sub_image, current_divide, device)
        style_image[image_roi[0]:image_roi[1], image_roi[2]:image_roi[3], :] = \
            sub_style_image[-offset[0]:sub_style_image.shape[0]-offset[1], -offset[2]:sub_style_image.shape[1]-offset[3]]
    return style_image
...
def calculate_image_statistics(image):
    ...
    return {"mean_l": mean_l, "std_l": std_l, "mean_a": mean_a, "std_a": std_a, "mean_b": mean_b, "std_b": std_b}
